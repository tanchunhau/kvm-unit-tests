...
{
	printf("--- Test 05 (negative) - Get domain attributes with wrong domain id\n");
	int ret = MPXY_TEST_SUCCESS;
	struct sbiret sret;
	unsigned long rx_bytes;
	struct rpmi_pm_get_domain_attrs_tx pm_get_domain_attrs_tx;
...
int device_power_set_state_with_wrong_domain_id(struct sbi_mpxy* mpxy)
{
	...
	pm_set_power_state_tx.domain_id = EXPECTED_NUMBER_OF_DOMAIN + 1;
...
int device_power_set_state_with_wrong_power_state(struct sbi_mpxy* mpxy)
{
	...
	for (u32 i = 0; i < EXPECTED_NUMBER_OF_DOMAIN; i++) {
		pm_set_power_state_tx.domain_id = i;
		pm_set_power_state_tx.power_state = RMPI_POWER_STATE_RESERVED0;
		memcpy(mpxy->shmem, &pm_set_power_state_tx, sizeof(pm_set_power_state_tx));
...
void run_device_power_test(struct sbi_mpxy* mpxy)
{
	...
	device_power_set_state_with_wrong_domain_id(mpxy);
	device_power_set_state_with_wrong_power_state(mpxy);
}
